...
import "whatwg-fetch";
// import '../../../profill/acstatic-dun';
import ServerConfig from '@/config/ServerConfig'
import WebSocketConfig from '@/config/WebSocketConfig'
import {resolveHelpPath, goHelpPath} from '@/config/UrlConfig'
...
const renderDom = async Component => {
  ...
  if (pathname.includes('/activity')) {
    goActivityPath(`/register/${window.location.search}`)
    return
  }
  render(
      <PcAppContianer>
...
// if (Device.mobile()) {
//   import (/* webpackChunkName: "AppMb" */ '@/componentsMb/home/Home.jsx').then(Component => renderDom(Component.default))
// } else {
//   if (!device || version < broFlagObj[device]) {
//     alert('您的浏览器不支持部分功能，请升级或更换浏览器!\nYour browser does not support some functions, please upgrade or replace the browser!')
//   }
//   import (/* webpackChunkName: "AppMb" */ './pc').then(Component => renderDom(Component.default))
// }
