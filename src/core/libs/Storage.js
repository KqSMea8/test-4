...
export default {
   ...
   * 设置cookie
   */
  setCookie(key, value, domain, path = '/', time = false) {
    if (!key) return false;
    let data = {
      value,
      domain,
    ...
    document.cookie = `${key}=${JSON.stringify(data)}; expires=${expires}; domain=${domain}; path=${path}`;
    return true
  },
   ...
   */
  getCookie(key) {
    let data = this.getCookieAll(key);
    if (!data || data === "null") return null;
    return data.value
  },
   ...
   */
  getCookieAll(key) {
    let arr, reg=new RegExp("(^| )"+key+"=([^;]*)(;|$)"),obj;
    if(arr = document.cookie.match(reg)){
      try {
        obj = JSON.parse(unescape(arr[2]));
      } catch (e) {
        return null;
      }
      return obj;
   ...
   * 删除存入localStorage中的某一个
   */
  delCookie(key, value, arrPath = []) {
    if (!value || !key) return false
    let dataAll = this.getCookieAll(key), data = dataAll.value, tmpData = dataAll.value, domain = dataAll.domain, path = dataAll.path, time = dataAll.time,
        valueTmp = [value]
    arrPath instanceof Array && arrPath.length > 0 && arrPath.forEach(v => tmpData = tmpData[v])
    tmpData instanceof Array && value instanceof Object && tmpData.forEach(v => {
      let valueFlag = 0
    ...
    tmpData instanceof Array && valueTmp.forEach(v => tmpData.indexOf(v) >= 0 && tmpData.splice(0, tmpData.length, ...tmpData.filter(vv => vv != v)))
    this.setCookie(key, data, domain, path, time)
    return true
  },
   ...
   * 添加localStorage
   */
  addCookie(key, value, index = 0, arrPath = []) {
    if (!value || !key) return false
    let dataAll = this.getCookieAll(key), data = dataAll.value, tmpData = dataAll.value, domain = dataAll.domain, path = dataAll.path, time = dataAll.time;
    arrPath instanceof Array && arrPath.length > 0 && arrPath.forEach(v => tmpData = tmpData[v])
    tmpData instanceof Array && tmpData.splice(index, 0, value)
   ...
   * 删除某个cookie
   */
  removeCookie(key, domain,  path = '/', time = false) {
    // console.log('delCookie', key, domain, time, path);
    if (!key) return false;
