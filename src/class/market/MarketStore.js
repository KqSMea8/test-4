  ...
  constructor(name) {
    ...
    if (name === 'market') {
      ...
      this.WebSocket.general.on('marketPair', data => {
        // this.Logger('getWebSocketData marketPair', data, this.controller)
...
// 汇率变化更新
  ...
  async selectMarketData() {
    ...
    // console.log(1232, this.Storage.userToken.get())
  ...
  //币种资料
  async getCoinInfo(coinName) {
    let r = await this.Proxy.coinInfo({cn: coinName});
