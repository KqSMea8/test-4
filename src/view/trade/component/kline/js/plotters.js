    ...
    Draw(context) {
        ...
        if (selIndex > 0) {
            ...
            if (change.toFixed(2) >= 0) {
                change = Math.abs(change.toFixed(2)) ? ' ' + Math.abs(change.toFixed(2)) : ' ' + '0.00';
        ...
        for (n = 0; n < cnt; n++) {
            ...
            let num = Number(v.toFixed(digits)) === 0 ? '0.000000' : v.toFixed(digits);
            let info = "  " + out.getName() + ": " + num;
...
export class IndicatorInfoPlotter extends Plotter {
    ...
    Draw(context) {
        ...
        for (n = 0; n < cnt; n++) {
            ...
            if (isNaN(v))
            continue;
            info = "  " + out.getName() + ": " +( Number(v.toFixed(2)) === 0 ? '0.00' : v.toFixed(2));
            color = out.getColor();
            if (color === undefined)
            color = themes.Theme.Color.Indicator0 + n;
            context.fillStyle = theme.getColor(color);
...
export class RangePlotter extends NamedObject {
    ...
    Draw(context) {
        ...
        for (let n in gradations) {
            ...
            context.fillText(Util.fromFloat(gradations[n], YaxisScale) , center, y);
        }
        if (gridRects.length > 0) {
            context.fillStyle = theme.getColor(themes.Theme.Color.Grid1);
            Plotter.createRectangles(context, gridRects);
